    ...
    for perms in (5..10).permutations() {
        ...
        'l: loop {
            for (i, a) in amplifiers.iter_mut().enumerate() {
                let loc = a.output.len();
                let resp = a.run(vec![v]);
                if loc == a.output.len() {
                    assert!(resp == RunStatus::NoOutput);
                    assert_eq!(0, i);
                    break 'l;
                }
                assert_eq!(loc + 1, a.output.len());
                v = a.output[loc];
            }
        }
        max = max.max(v);
